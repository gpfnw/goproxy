# Usage: https://hub.docker.com/r/gpfnw/goproxy-vps/

FROM alpine:3.6

RUN apk add --no-cache curl xz tar && \
	mkdir /tmp/goproxy-vps && \
	goproxy_vps_loc=$(curl -Lks https://github.com/gpfnw/goproxy-ci/releases/ | grep -oE '/gpfnw/goproxy-ci/.*/goproxy-vps_linux_amd64-r[0-9]+.tar.xz' | head -1) && \
