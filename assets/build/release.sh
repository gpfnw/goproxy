...
export GITHUB_USER_EMAIL=${GITHUB_USER_EMAIL:-gpfnw@hotmail.com}
...
RELEASE_URL=https://github.com/gpfnw/goproxy-ci/releases/tag/${GITHUB_TAG}
else
RELEASE_URL=https://github.com/gpfnw/goproxy-ci/releases/latest
fi

RELEASE_FILES=$(curl -ksSL ${RELEASE_URL} | grep -oP '(goproxy|source)[^/]*\.(tar|gz|bz2|xz|7z|zip)' | uniq)
RELEASE_MESSAGE="https://github.com/gpfnw/goproxy-ci/releases/tag/$(echo ${RELEASE_FILES} | grep -oP 'r\d\d\d\d' | head -1)"
