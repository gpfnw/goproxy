...
export GITHUB_USER=${GITHUB_USER:-gpfnw}
export GITHUB_EMAIL=${GITHUB_EMAIL:-gpfnw@hotmail.com}
...
function build_go() {
	...
	git clone --branch master https://github.com/gpfnw/go
	cd go/src
...
function build_glog() {
	...
	git clone https://github.com/gpfnw/glog $GOPATH/src/github.com/gpfnw/glog
	cd $GOPATH/src/github.com/gpfnw/glog
...
function build_http2() {
	...
	git clone https://github.com/gpfnw/net $GOPATH/src/github.com/gpfnw/net
	cd $GOPATH/src/github.com/gpfnw/net/http2
	git remote add -f upstream https://github.com/golang/net
	git rebase upstream/master
	go get -x github.com/gpfnw/net/http2
	grep -q 'machine github.com' ~/.netrc && git push -f origin master
...
function build_quicgo() {
	...
	git clone https://github.com/gpfnw/quic-go $GOPATH/src/github.com/gpfnw/quic-go
	cd $GOPATH/src/github.com/gpfnw/quic-go
	git remote add -f upstream https://github.com/lucas-clemente/quic-go
	git rebase upstream/master
	go get -v github.com/gpfnw/quic-go/h2quic
	grep -q 'machine github.com' ~/.netrc && git push -f origin master
...
function build_goproxy_gae() {
	...
	do
		curl -LOJ https://raw.githubusercontent.com/gpfnw/pybuild/master/${FILE}
	done
...
function build_goproxy_vps() {
	...
	git clone --branch master https://github.com/gpfnw/goproxy $GOPATH/src/github.com/gpfnw/goproxy
	awk 'match($1, /"((github\.com|golang\.org|gopkg\.in)\/.+)"/) {if (!seen[$1]++) {gsub("\"", "", $1); print $1}}' $(find . -name "*.go") | xargs -n1 -i go get -u -v {}
